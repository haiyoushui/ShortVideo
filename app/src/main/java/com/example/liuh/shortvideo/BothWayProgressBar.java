...
public class BothWayProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mVisibility == View.VISIBLE) {
            ...
            //画出进度条
            if (progress < 1000){
                canvas.drawRect(mid*(float)progress/(float)1000, 0, width-mid*(float)progress/(float)1000, height, isCancel ? mCancelPaint : mRecordPaint);
            } else {
